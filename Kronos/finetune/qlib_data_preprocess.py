...
class QlibDataPreprocessor:
    ...
    def load_qlib_data(self):
        ...
        print(f"Loading data for instrument: {self.config.instrument}")
        ...
        try:
            ...
            print("Trying with a smaller instrument set...")
            # 尝试使用单个股票
            try:
                data_df = QlibDataLoader(config=data_fields_qlib).load(
                    ['SH600000'], real_start_time, real_end_time
                )
                print(f"Single stock data loaded, shape: {data_df.shape}")
                data_df = data_df.stack().unstack(level=1)  # Reshape for easier access.
            except Exception as e2:
                print(f"Error loading single stock data: {e2}")
                return None
